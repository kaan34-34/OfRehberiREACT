...
import { businesses, neighborhoods, categories } from '../data/mockData';
...
export default function HomePage() {
  ...
  const getFilteredBusinesses = () => {
    ...
    if (filters.delivery) {
      filtered = filtered.filter((b) => b.hasDelivery);
    }

    if (filters.acceptsCard) {
      filtered = filtered.filter((b) => b.acceptsCard);
    }
  ...
  return (
    <Box>
              ...
              <Grid size={{ xs: 12, md: 9 }}>
                ...
                <Typography variant="body2" sx={{ mb: 2, color: '#666' }}>
                  {selectedCategoryName && (
                    <strong style={{ color: '#7f1810' }}>{selectedCategoryName}</strong>
                  )}{' '}
                  {searchQuery && (
                    <>
      ...
      {/* Discover Section */}
      <DiscoverSection onTagClick={handleCategorySelect} />
    </Box>
  );
}
